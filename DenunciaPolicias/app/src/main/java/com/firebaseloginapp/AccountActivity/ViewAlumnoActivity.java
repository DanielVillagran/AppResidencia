...
public class ViewAlumnoActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ncontrol.setText(extras.getString("ncontrol"));
        ...
        int contador=0;
        for(String g : list){
            if(g.equals(extras.getString("carrera"))){
                carrera.setSelection(contador);
            }
            contador++;
        }
        ...
        contador=0;
        for(String g : list){
            if(g.equals(extras.getString("semestre"))){
                semestre.setSelection(contador);
            }
            contador++;
        }
    ...
    public void registrar_alumno(View v) {
        ...
        authAlumnos.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(ViewAlumnoActivity.this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        authAlumnos.signOut();
                        Toast.makeText(ViewAlumnoActivity.this, "Alumno Creado!" , Toast.LENGTH_SHORT).show();
                        progressBar.setVisibility(View.GONE);
                        // If sign in fails, display a message to the user. If sign in succeeds
                        // the auth state listener will be notified and logic to handle the
                        // signed in user can be handled in the listener.
                        if (!task.isSuccessful()) {
                            Toast.makeText(ViewAlumnoActivity.this, "Authentication failed." + task.getException(),
